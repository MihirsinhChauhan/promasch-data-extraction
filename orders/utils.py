...
def setup_logging(name: str = "orders") -> logging.Logger:
...
def save_failed_id(path: Path, order_id: int | str, error: str) -> None:
    with _failed_lock:
        failed = load_failed_ids(path)
        if "po" not in failed:
            failed["po"] = []
        existing_ids = {entry["order_id"] for entry in failed["po"]}
...
def get_failed_order_ids(path: Path) -> list[str]:
    failed = load_failed_ids(path)
    return [entry["order_id"] for entry in failed.get("po", [])]


def clear_failed_ids(path: Path, order_ids: list[str]) -> None:
    with _failed_lock:
        failed = load_failed_ids(path)
        remove_set = set(order_ids)
        failed["po"] = [e for e in failed.get("po", []) if e["order_id"] not in remove_set]
        save_json(path, failed)
