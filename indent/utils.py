...
def setup_logging(name: str = "indent") -> logging.Logger:
...
def save_failed_id(path: Path, indent_id: str, error: str) -> None:
    with _failed_lock:
        failed = load_failed_ids(path)
        key = "indent"
        if key not in failed:
            failed[key] = []
        existing_ids = {entry["indent_id"] for entry in failed[key]}
        if str(indent_id) not in existing_ids:
            failed[key].append({
...
def get_failed_indent_ids(path: Path) -> list[str]:
    failed = load_failed_ids(path)
    return [entry["indent_id"] for entry in failed.get("indent", [])]


def clear_failed_ids(path: Path, indent_ids: list[str]) -> None:
    with _failed_lock:
        failed = load_failed_ids(path)
        remove_set = set(indent_ids)
        failed["indent"] = [
            e for e in failed.get("indent", [])
            if e["indent_id"] not in remove_set
        ]
        save_json(path, failed)
